...
const DepositFlow = ({
  ...
  const {
    dispatch, handleConvertedAmountChange, handleDepositAmountChange,
    selectBankAccount, selectDerivAccount, state, setStep,
    ...
    try {
      ...
      if (response.success) {
        if (response.data?.transactionId) {
          ...
          dispatch({ type: "SET_ERROR", payload: "Transaction ID is missing." });
        }
  ...
  return step === 1 ? <StepSelectDerivAccount
    onNext={setStep}
    selectedDerivAccount={state.selectedDerivAccount}
    derivAccountsRes={derivAccountsRes} onSelect={selectDerivAccount}
  /> : step === 2 ? <StepSelectBankAccount
    onNext={setStep}
    selectedBankAccount={state.selectedBankAccount}
    bankAccountsRes={bankAccountsRes} onSelect={selectBankAccount}
  /> : <StepConfirm
    state={state} onSubmit={onSubmit}
    onDepositChange={handleDepositAmountChange} onConvertedChange={handleConvertedAmountChange}
  />;
};
